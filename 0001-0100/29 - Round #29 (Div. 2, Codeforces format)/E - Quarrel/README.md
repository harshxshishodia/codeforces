...
**Submission:** https://codeforces.com/contest/29/problem/E

**Limits:** 1 second / 256 megabytes

## Problem Statement

Friends Alex and Bob live in Bertown. In this town there are n crossroads, some of them are connected by bidirectional roads of equal length. Bob lives in a house at the crossroads number 1 , Alex ΓÇö in a house at the crossroads number n .
...
The first line contains two integers n and m ( 2ΓÇëΓëñΓÇë n ΓÇëΓëñΓÇë500,ΓÇë1ΓÇëΓëñΓÇë m ΓÇëΓëñΓÇë10000 ) ΓÇö the amount of crossroads and the amount of roads. Each of the following m lines contains two integers ΓÇö the numbers of crossroads connected by the road. It is guaranteed that no road connects a crossroads with itself and no two crossroads are connected by more than one road.

## Output

If the required routes don't exist, output -1 . Otherwise, the first line should contain integer k ΓÇö the length of shortest routes (the length of the route is the amount of roads in it). The next line should contain k ΓÇë+ΓÇë1 integers ΓÇö Bob's route, i.e. the numbers of k ΓÇë+ΓÇë1 crossroads passed by Bob. The last line should contain Alex's route in the same format. If there are several optimal solutions, output any of them.
