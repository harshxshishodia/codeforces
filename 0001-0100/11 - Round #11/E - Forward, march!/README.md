...
The first line of input contains a sequence consisting only of characters ' L ', ' R ' and ' X ', where ' L ' corresponds to a step with the left foot, ' R ' ΓÇö with the right foot, and ' X ' ΓÇö to a break. The length of the sequence will not exceed 10 6 .
...
In the second example, if we add two breaks to receive LXXRXR , Jack will march: LXXRXRLXXRXRL ... instead of LRLRLRLRLRLRL ... and will make the correct step in half the cases. If we didn't add any breaks, the sequence would be incorrect ΓÇö Jack can't step on his right foot twice in a row.
