...
**Submission:** https://codeforces.com/contest/93/problem/C
...
Having carefully thumbed through the book, Igor K. realised that, as it turns out, you can multiply the numbers dozens of times faster. "Search Ultimate will be faster than it has ever been!" ΓÇö the fellow shouted happily and set to work.
...
- [ x ] ΓÇö returns the value located in the address x . For example, [eax] returns the value that was located in the address, equal to the value in the register eax. 
- lea x , y ΓÇö assigns to the register x , indicated as the first operand, the second operand's address. Thus, for example, the "lea ebx, [eax]" command will write in the ebx register the content of the eax register: first the [eax] operation will be fulfilled, the result of it will be some value that lies in the address written in eax. But we do not need the value ΓÇö the next operation will be lea, that will take the [eax] address, i.e., the value in the eax register, and will write it in ebx. 
...
The input data contain the only integer n ( 1ΓÇëΓëñΓÇë n ΓÇëΓëñΓÇë255 ), which Igor K. is about to multiply.
