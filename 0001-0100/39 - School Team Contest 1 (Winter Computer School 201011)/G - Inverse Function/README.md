...
**Submission:** https://codeforces.com/contest/39/problem/G
...
- operatorSequence ::= operator ┬á|┬á operator ┬á operatorSequence 
- operator ::= return arithmExpr ; | if ( logicalExpr ) return arithmExpr ;
- logicalExpr ::= arithmExpr ΓÇë>ΓÇë arithmExpr | arithmExpr ΓÇë<ΓÇë arithmExpr | arithmExpr == arithmExpr 
- arithmExpr ::= sum 
- sum ::= product | sum ΓÇë+ΓÇë product | sum ΓÇë-ΓÇë product 
- product ::= multiplier | product ΓÇë*ΓÇë multiplier | product ΓÇë/ΓÇë multiplier 
- multiplier ::= n | number | f( arithmExpr )
- number ::= 0|1|2|... |32767 

The whitespaces in a operatorSequence are optional.

Thus, we have a function, in which body there are two kinds of operators. There is the operator "return arithmExpr ;" that returns the value of the expression as the value of the function, and there is the conditional operator "if ( logicalExpr ) return arithmExpr ;" that returns the value of the arithmetical expression when and only when the logical expression is true. Guaranteed that no other constructions of C++ language ΓÇö cycles, assignment operators, nested conditional operators etc, and other variables except the n parameter are used in the function. All the constants are integers in the interval [0..32767] .
...
2. When performing a function for the value n ΓÇë=ΓÇë N recursive calls of the function f may occur only for the parameter value of 0,ΓÇë1,ΓÇë...,ΓÇë N ΓÇë-ΓÇë1 . Consequently, the program never has an infinite recursion.
...
The first line has an integer f ( n ) from the interval [0..32767] . The next lines have the description of the function f . In the description can be found extra spaces and line breaks (see the examples) which, of course, canΓÇÖt break key words int, if, return and numbers. The size of input data canΓÇÖt exceed 100 bytes.

## Output

Output a single number ΓÇö the answer to the problem. If thereΓÇÖs no answer, output "-1" (without quotes).
