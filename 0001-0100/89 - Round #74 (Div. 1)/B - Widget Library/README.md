...
**Submission:** https://codeforces.com/contest/89/problem/B
...
Types HBox and VBox are derivatives of type Widget , so they also are types Widget . Widgets HBox and VBox are special. They can store other widgets. Both those widgets can use the pack() method to pack directly in itself some other widget. Widgets of types HBox and VBox can store several other widgets, even several equal widgets ΓÇö they will simply appear several times. As a result of using the method pack() only the link to the packed widget is saved, that is when the packed widget is changed, its image in the widget, into which it is packed, will also change. 

We shall assume that the widget a is packed in the widget b if there exists a chain of widgets a ΓÇë=ΓÇë c 1 ,ΓÇë c 2 ,ΓÇë...,ΓÇë c k ΓÇë=ΓÇë b , k ΓÇëΓëÑΓÇë2 , for which c i is packed directly to c i ΓÇë+ΓÇë1 for any 1ΓÇëΓëñΓÇë i ΓÇë<ΓÇë k . In Vasya's library the situation when the widget a is packed in the widget a (that is, in itself) is not allowed. If you try to pack the widgets into each other in this manner immediately results in an error.
...
The picture above shows how the widgets are packed into HBox and VBox . At that HBox and VBox automatically change their size depending on the size of packed widgets. As for HBox and VBox , they only differ in that in HBox the widgets are packed horizontally and in VBox ΓÇö vertically. The parameter spacing sets the distance between adjacent widgets, and border ΓÇö a frame around all packed widgets of the desired width. Packed widgets are placed exactly in the order in which the pack() method was called for them. If within HBox or VBox there are no packed widgets, their sizes are equal to 0ΓÇë├ùΓÇë0 , regardless of the options border and spacing . 
...
The first line contains an integer n ΓÇö the number of instructions ( 1ΓÇëΓëñΓÇë n ΓÇëΓëñΓÇë100 ). Next n lines contain instructions in the language VasyaScript ΓÇö one instruction per line. There is a list of possible instructions below. 

- " Widget [name]([x],[y]) " ΓÇö create a new widget [name] of the type Widget possessing the width of [x] units and the height of [y] units. 
- " HBox [name] " ΓÇö create a new widget [name] of the type HBox . 
- " VBox [name] " ΓÇö create a new widget [name] of the type VBox . 
- " [name1].pack([name2]) " ΓÇö pack the widget [name2] in the widget [name1] . At that, the widget [name1] must be of type HBox or VBox . 
- " [name].set_border([x]) " ΓÇö set for a widget [name] the border parameter to [x] units. The widget [name] must be of type HBox or VBox . 
- " [name].set_spacing([x]) " ΓÇö set for a widget [name] the spacing parameter to [x] units. The widget [name] must be of type HBox or VBox . 
