...
**Submission:** https://codeforces.com/contest/57/problem/D
...
The first line contains two space-separated integers: n ,ΓÇë m ( 2ΓÇëΓëñΓÇë n ,ΓÇë m ΓÇëΓëñΓÇë1000 ) which represent the sizes of the universe. The next n lines containing m symbols each describe the universe without dynamic particles ΓÇö the j -th symbol of the i -th line equals to 'X' if the cell is occupied by a static particle, and to '.' if it is empty. It is guaranteed that the described universe satisfies the properties described above, that is no two static particles can be in one column or in one row, besides, they can't be positioned in the diagonally adjacent cells.
...
The answer will be accepted if it is within 10 ΓÇë-ΓÇë6 of absolute or relative error from the correct answer.
