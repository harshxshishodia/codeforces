...
**Submission:** https://codeforces.com/contest/39/problem/I

**Limits:** 2 seconds / 64 megabytes

## Problem Statement

In a Berland city S*** there is a tram engine house and only one tram. Three people work in the house ΓÇö the tram driver, the conductor and the head of the engine house. The tram used to leave the engine house every morning and drove along his loop route. The tram needed exactly c minutes to complete the route. The head of the engine house controlled the tramΓÇÖs movement, going outside every c minutes when the tram drove by the engine house, and the head left the driver without a bonus if he was even one second late.
...
The city people were rejoicing for some of the had been waiting for the tram to appear on their streets for several years. However, the driverΓÇÖs behavior enraged the engine house head. Now he tries to carry out an insidious plan of installing cameras to look after the rebellious tram.
...
The first line contains integers n and m ( 2ΓÇëΓëñΓÇë n ,ΓÇë m ΓÇëΓëñΓÇë10 5 ) ΓÇö the number of crossroads and tramlines in S*** respectively. The next m lines contain the descriptions of the tramlines in " u v " format, where u is the initial tramline crossroad and v is its final crossroad. The crossroads are numbered with integers from 1 to n , and the engine house is at the crossroad number 1 .

## Output

In the first line output the value of t . In the next line output the value of k ΓÇö the required number of the cameras. In the next line output space-separated numbers of the crossroads, where the cameras should be installed. Output the numbers in increasing order.
